...
[![VS Code Marketplace](https://img.shields.io/vscode-marketplace/v/choshinyoung.toast-vscode?label=VS%20Code%20Marketplace&logo=visualstudiocode)](https://marketplace.visualstudio.com/items?itemName=choshinyoung.toast-vscode)
...
VS Code 마켓플레이스 또는 확장 탭에서 **`Toast Programming Language`**를 검색하여 설치할 수 있습니다. 문법 하이라이팅 및 Language Server(LSP) 기반 실시간 진단/자동완성을 제공합니다.
...
전역 도구 설치:
```bash
dotnet tool install -g choshinyoung.Toast.Tools --version 2.0.3-beta
```
...
라이브러리 설치:
```bash
dotnet add package choshinyoung.Toast --version 2.0.3-beta
```
